...
void DecisionTreeBase::fit(DataFrame *trainData, DecisionNode *&node) {
  ...
  if (trainData->rows() >= minSamplesSplit) {
    ...
    if (splitable) {
      ...
      node = new DecisionNode(nodeLoss, truthVector, trainData->rows(), splitColumn, splitValue);
      fit(half1, node->left);
      ...
      node = new DecisionNode(nodeLoss, truthVector, trainData->rows());
    }
  } else {
    node = new DecisionNode(nodeLoss, truthVector, trainData->rows());
  }
...
void DecisionTreeBase::balance(DecisionNode *&node) { // starting from root
  node->updateValues();

  if (node->balanceFactor() >= 2 and
      node->left->balanceFactor() >= 0) // left - left
  ...
  else if (node->balanceFactor() <= -2 and
           node->right->balanceFactor() <= 0) // right - right
...
void DecisionTreeBase::fit(DataFrame *trainData) { fit(trainData, root); }
...
void DecisionTreeBase::printTree(DecisionNode *node, int indents) {
  if (node != nullptr) {
    ...
    printSpaces(indents);
    cout << "sampleSize: " << node->count << endl << endl;

    printTree(node->left, indents + 1);
  }
}

void DecisionTreeBase::printTree() { printTree(root, 0); }